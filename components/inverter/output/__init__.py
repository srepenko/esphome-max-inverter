...
DEPENDENCIES = ["inverter"]

InverterOutput = inverter_ns.class_("InverterOutput", output.FloatOutput)
SetOutputAction = inverter_ns.class_("SetOutputAction", automation.Action)
...
CONFIG_SCHEMA = INVERTER_COMPONENT_SCHEMA.extend(
    {
        cv.Optional(type): output.FLOAT_OUTPUT_SCHEMA.extend(
            {
                cv.Required(CONF_ID): cv.declare_id(InverterOutput),
...
def output_inverter_set_level_to_code(config, action_id, template_arg, args):
